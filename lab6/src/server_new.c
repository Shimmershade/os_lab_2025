...
int main(int argc, char **argv) {
  ...
  while (true) {
    ...
    while (true) {
      ...
      // Разделение работы между потоками на сервере
      ...
      for (uint32_t i = 0; i < tnum; i++) {
        ...
        args[i].end = current_begin + step - 1;
        if (i < remainder) {
          args[i].end++;
        }
        args[i].mod = mod;
        ...
        if (pthread_create(&threads[i], NULL, ThreadFactorial,
                           (void *)&args[i])) {
          printf("Error потоки не созданы\n");
          return 1;
        }
      }
